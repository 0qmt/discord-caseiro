...
export function useVoice(socket) {
  ...
  const actions = useMemo(() => ({
    ...
    desbloquearVolumeAlto: (socketId) => clientRef.current?.desbloquearVolumeAlto(socketId),
    bloquearVolumeAlto: (socketId) => clientRef.current?.bloquearVolumeAlto(socketId),
    volumeAltoLiberado: (socketId) => clientRef.current?.volumeAltoLiberado(socketId) ?? false,
